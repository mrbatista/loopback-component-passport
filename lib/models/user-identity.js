 ...
 */
 module.exports = function(UserIdentity) {
  ...
  function identifyUserByQuery(userObj, options) {
   ...
   *
   * -  isNewInstance: identify if user is a new instance
  ...
  UserIdentity.login = function (provider, authScheme, profile, credentials,
    ...
    userIdentityModel.findOne({where: {
          ...
          return identity.user(function (err, user) {
            ...
            if(!err && user && autoLogin) {
              return (options.createAccessToken || createAccessToken)(user, function(err, token) {
                cb(err, user, false, identity, token);
              });
            }
            cb(err, user, false, identity);
          });
        });
      }
      // Find the user model
      ...
      var query = (options.identifyUserByQuery || identifyUserByQuery)(userObj, options);
      ...
      userModel.findOrCreate({ where: query }, userObj, function (err, user, created) {
        ...
        userIdentityModel.create({
          ...
          if(!err && user && autoLogin) {
            return (options.createAccessToken || createAccessToken)(user, function(err, token) {
              cb(err, user, created, identity, token);
            });
          }
          cb(err, user, created, identity);
        });
      });
    });
  };
