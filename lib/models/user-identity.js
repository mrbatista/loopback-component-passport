...
function UserIdentity(UserIdentity) {
  ...
  * -  isNewInstance: boolean variable that identify if model is new instance
  ...
  */
  UserIdentity.login = function(provider, authScheme, profile, credentials,
    ...
    userIdentityModel.findOne({where: {
          ...
          return identity.user(function(err, user) {
            ...
            if (!err && user && autoLogin) {
              return (options.createAccessToken || createAccessToken)(user, function(err, token) {
                cb(err, user, identity, false, token);
              });
            }
            cb(err, user, identity, false);
          });
        });
      }
      // Find the user model
      ...
      userModel.findOrCreate({where: query}, userObj, function(err, user, isNewInstance) {
        if (err) {
          return cb(err);
        }
        var date = new Date();
        userIdentityModel.findOrCreate({where: {externalId: profile.id}}, {
          ...
          if (!err && user && autoLogin) {
            return (options.createAccessToken || createAccessToken)(user, function(err, token) {
              cb(err, user, identity, isNewInstance, token);
            });
          }
          cb(err, user, identity, isNewInstance);
        });
      });
    });
  };
